...
public class IndicatorsController extends Controller {
    ...
    @FXML private MFXListView<String> listTopClients;
    @FXML private MFXListView<String> listTopBranches;
    @FXML private MFXListView<String> listTopStations;
    @FXML private MFXListView<String> listTopProcedures;
    @FXML private MFXListView<String> listTopPreferentials;

    @FXML private BarChart<String, Number> barChartClients;
    @FXML private BarChart<String, Number> barChartBranches;
    @FXML private BarChart<String, Number> barChartStations;
    @FXML private BarChart<String, Number> barChartProcedures;
    @FXML private BarChart<String, Number> barChartPreferentials;

    @FXML private TextField    tfC_Name;
    @FXML private TextField    tfC_Id;
    private TextField    tfC_Branch;
    @FXML private TextField    tfC_Procedure;
    @FXML private DatePicker   dpC_DateFrom;
    @FXML private DatePicker   dpC_DateTo;
    @FXML private ToggleButton toggleC_Preferential;

    @FXML private TextField    tfS_Branch;
    @FXML private DatePicker   dpS_DateFrom;
    @FXML private DatePicker   dpS_DateTo;
    @FXML private ToggleButton toggleS_Stations;
    @FXML private ToggleButton toggleS_Procedures;
    @FXML private ToggleButton toggleS_Preferential;

    @FXML private TextField    tfCJ_Station;
    @FXML private TextField    tfCJ_Branch;
    @FXML private TextField    tfCJ_Procedure;
    @FXML private ToggleButton toggleCJ_Preferential;

    @FXML private TextField    tfT_Procedure;
    @FXML private TextField    tfT_Branch;
    @FXML private DatePicker   dpT_DateFrom;
    @FXML private DatePicker   dpT_DateTo;
    @FXML private ToggleButton toggleT_Active;

    @FXML private TextField  tfP_Branch;
    @FXML private TextField  tfP_Procedure;
    @FXML private DatePicker dpP_DateFrom;
    @FXML private DatePicker dpP_DateTo;
    ...
    private void loadData() {
        ...
        Type ticketType   = new TypeToken<List<Ticket>>() {}.getType();
    ...
    private void addTextListener(TextField tf, Runnable action) {
        if (tf != null) tf.textProperty().addListener((obs, ov, nv) -> action.run());
    }

    private void addDateListener(DatePicker dp, Runnable action) {
        if (dp != null) dp.valueProperty().addListener((obs, ov, nv) -> action.run());
    }
    ...
    private void refreshActiveTab() {
        ...
        switch (tabPane.getSelectionModel().getSelectedIndex()) {
            case 0 -> refreshTabClients();
            case 1 -> refreshTabBranches();
            case 2 -> refreshTabStations();
            case 3 -> refreshTabProcedures();
            case 4 -> refreshTabPreferentials();
            default -> refreshAllTabs();
        }
    }
    ...
    private void renderClientList(List<Client> result, MFXListView<String> list,
        ...
        if (items.isEmpty()) items.add(emptyMessage);
    ...
    private void renderBranchList(List<Branch> result) {
        ...
        if (items.isEmpty()) items.add("No hay sucursales con los filtros seleccionados.");
    ...
    private void renderStationList(List<Station> result) {
        ...
        if (items.isEmpty()) items.add("No hay cajas con los filtros seleccionados.");
    ...
    private void renderProcedureList(List<Procedure> result) {
        ...
        if (items.isEmpty()) items.add("No hay trámites con los filtros seleccionados.");
    ...
    private boolean isInDateRange(String creationDate, LocalDate from, LocalDate to) {
        if (creationDate == null || creationDate.length() < 10) return false;
        try {
    ...
    private void OnActionFilterDates(ActionEvent event)        { refreshActiveTab(); }
    private void OnActionFilterBranches(ActionEvent event)     { refreshActiveTab(); }
    @FXML private void OnActionFilterStations(ActionEvent event)     { refreshActiveTab(); }
    @FXML private void OnActionFilterProcedures(ActionEvent event)   { refreshActiveTab(); }
    @FXML private void OnActionFilterPreferential(ActionEvent event) { refreshActiveTab(); }
    private void OnActionFilterName(ActionEvent event)         { refreshTabClients(); }
    private void OnActionFilterId(ActionEvent event)           { refreshTabClients(); }
}
