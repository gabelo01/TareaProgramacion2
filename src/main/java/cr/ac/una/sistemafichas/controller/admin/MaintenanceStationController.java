...
public class MaintenanceStationController extends Controller {
    ...
    @FXML private CheckBox          chkIsStationActive;
    ...
    private void setupFilterListeners() {
        if (tfCJ_Station != null) tfCJ_Station.textProperty().addListener((obs, ov, nv) -> applyFilter());
    }

    private void applyFilter() {
        String filter = tfCJ_Station != null ? tfCJ_Station.getText().trim().toLowerCase() : "";
        List<Station> result = filter.isEmpty()
            ? new ArrayList<>(allStations)
            : allStations.stream()
                .filter(st -> st.getName() != null && st.getName().toLowerCase().contains(filter))
                .collect(Collectors.toList());
        tblListStations.getItems().setAll(result);
    }
    ...
    private void loadBranches() {
        ...
        cmbBranches.getItems().setAll(branches.stream().map(Branch::getName).toList());
        ...
        cmbBranches.setOnAction(e -> {
            ...
            if (currentBranch != null) {
                allStations = currentBranch.getStations() != null
                    ? currentBranch.getStations() : new ArrayList<>();
    ...
    private void loadProcedures() {
        ...
        allProcedures = allProcedures.stream().filter(Procedure::isActive).toList();
        ...
        tblListStations.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, st) -> {
            ...
            if (st != null) {
                ...
                List<Procedure> assigned = allProcedures.stream()
                    .filter(p -> st.getProcedureNames().contains(p.getName())).toList();
    ...
    private void onActionBtnSave(ActionEvent e) {
        ...
        if (selectedStation == null) {
            Station s = new Station(name, chkPreferential.isSelected(), currentBranch.getName(), procedures, chkIsStationActive.isSelected());
            stations.add(s);
        } else {
    ...
    private Procedure buscar(String name) {
        return allProcedures.stream().filter(p -> p.getName().equals(name)).findFirst().orElse(null);
    }
